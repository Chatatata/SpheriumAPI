...
#### Concrete authentication samples
...
##### Two-factor authentication over TBC
1. The server is challenged with user credentials, since device (or particular front-end application) is either
  * not registered yet, or
  * registered but its passphrase is expired.
...
#### Stealth authentication sample
1. The server is requested to generate a JWT.
...
2. Server is accessed from a protected route.
